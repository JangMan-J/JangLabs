...
# KB records that model several COMPLEMENTARY env vars (set together) under one id,
# rather than mutually-exclusive alternatives — e.g. HDR wants PROTON_ENABLE_HDR and
# DXVK_HDR both. The maximal launch string keeps every distinct key for these instead
# of collapsing them to a single representative.
ADDITIVE_GROUP_IDS = {"proton_enable_hdr"}
...
def render_markdown(con, summary, items, runtime, profile, priority, compare=None):
    ...
    # The pasteable launch-option strings are rendered at the very END of the report
    # (see "Full launch options — copy-paste") so it closes on the actionable result.
    ...
    # ---- FINAL: full launch-option strings (the actionable copy-paste TL;DR) ----
    chosen_env_wrap = [it for it in items if it["rec"]["tier"] == "recommended"
    ...
    A.append("")
    A.append("## ✅ Full launch options — copy-paste")
    A.append("")
    if _installed:
        A.append(f"**Proton runtime:** select `{_installed}` in *Properties → Compatibility* "
                 "(this is a dropdown, not part of the Launch Options box).")
    else:
        A.append(f"**Proton runtime:** {runtime['ge_label']} via *Properties → Compatibility* "
                 "(this is a dropdown, not part of the Launch Options box).")
    A.append("")
    A.append("**Recommended — the safe set (paste into Launch Options):**")
    A.append("```")
    A.append(launch)
    A.append("```")
    max_launch = assemble_launch(maximal_items(items, chosen_env_wrap), profile)
    # "What's new" = the exact tokens the maximal string adds over the safe one, so the
    # prose can never disagree with the string (covers env-sort, scheduler normalization).
    safe_tokens = set(launch.split())
    added = [t for t in max_launch.split() if t != "%command%" and t not in safe_tokens]
    if added:
        A.append("")
        A.append("**Maximal — also folds in every low-risk optional param "
                 f"({', '.join('`' + a + '`' for a in added)}):**")
        A.append("```")
        A.append(max_launch)
        A.append("```")
        A.append("> Maximal is opportunistic — it includes thin-evidence and *conditional* params "
                 "(e.g. HDR needs an HDR display + compositor HDR enabled; see the Optional section "
                 "above for each one's prerequisite). Use it only if you want everything; fall back "
                 "to the Recommended string if anything misbehaves.")
    else:
        A.append("")
        A.append("_No additional low-risk optional parameters apply here — the Recommended string is "
                 "already the maximal safe set for your hardware._")
    if warned:
        A.append("")
        A.append("**⚠️ Do NOT add to either string:**")
        for it in warned:
...
def _concept_key(p, kb_rec):
    """A dedup key coarser than the KB id for params that are the SAME operation
    expressed differently (so the maximal string keeps only one): the gamemode-vs-
    game-performance scheduler pair, and prime-run vs its underlying __NV_PRIME_*
    env (prime-run already sets it). Everything else dedups by KB id."""
    if kb_rec is None:
        return None
    if p["namespace"] == "wrapper" and p["key"] in ("gamemoderun", "game-performance"):
        return "concept:scheduler"
    if kb_rec.get("category") == "gpu-offload-hybrid":
        return "concept:gpu-offload"
    return kb_rec["id"]


def maximal_items(items, rec_items):
    """Recommended env/wrapper/arg items PLUS the low-risk Optional params that are
    safe to fold into a single pasteable string.

    Filtered to things that actually belong in a launch string: env/wrapper/arg only,
    KB-annotated, low effective-risk, beneficial (not elective/monitoring), not a
    negated/disabling form, not an anti-cheat flag, and no `<placeholder>` value.
    A concept (see _concept_key) already covered by the Recommended set — or by an
    earlier addition — is deduped out, EXCEPT complementary groups (HDR) whose
    distinct keys are meant to be set together. The caller derives the "what's new"
    list by diffing the assembled strings, so this needn't track displays."""
    seen = {_concept_key(it["p"], it["kb"]) for it in rec_items if it["kb"]}
    out = list(rec_items)
    for it in items:
        p, kb_rec, rec = it["p"], it["kb"], it["rec"]
        if rec["tier"] != "optional" or p["namespace"] not in ("env", "wrapper", "arg"):
            continue
        if kb_rec is None or rec["risk"] != "low":
            continue
        if not is_beneficial(rec["axes"]) or _negated(p):
            continue
        if kb_rec["category"] == "anti-cheat-or-drm":
            continue
        if "<" in p["display"]:                     # placeholder value (=<num>) — not pasteable
            continue
        concept = _concept_key(p, kb_rec)
        if concept in seen and concept not in ADDITIVE_GROUP_IDS:
            continue
        seen.add(concept)
        out.append(it)
    return out
